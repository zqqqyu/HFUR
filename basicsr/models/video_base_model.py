...
class VideoBaseModel(SRModel):
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx in range(rank, len(dataset), world_size):
          ...
          #  result_img = tensor2img(visuals['result1'])
            
           # Gt = result_img
           
          #  plt.imshow(result_img,cmap='GnBu_r')
         #   plt.colorbar()
        #    plt.show()
            
          #  image = tensor2img([visuals['gt']],rgb2bgr=False)
           # image= PIL.Image.fromarray(image)
           # print(image.shape)
            
           # map = cv2.applyColorMap(Gt,cv2.COLORMAP_JET)
           # plt.imshow(map)
           # plt.colorbar()
           # plt.show()
            
          #  map = cv2.cvtColor(map,cv2.COLOR_BGR2RGB)
           ## image.paste(map,mask=Gt)   #Image.fromarry(
           # image.save('test.png')
            ...
            #plt.imshow(result_img, cmap='GnBu_r')
            #plt.show()
            #plt.savefig('x.jpg',bbox_inches = 'tights',pad_inches = -0.1)
           ...
           # print(visuals['lq'].shape)
            ...
            if with_metrics:
                ...
                for metric_idx, opt_ in enumerate(opt_metric.values()):
                   ...
                   # print(result_img-lq_img)
                    result = getattr(metric_module,
                                     metric_type)(result_img, gt_img, **opt_)
                    result_o = getattr(metric_module,
                                     metric_type)(lq_img, gt_img, **opt_)
                    result_increase = result - result_o
               #     result_increase = result_o
                    self.metric_results[folder][int(frame_idx),
                                                metric_idx] += result_increase
