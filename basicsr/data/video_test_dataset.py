...
class VideoTestDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if opt['name'].lower() in ['vid4', 'reds4', 'redsofficial', 'ldv']:
            for subfolder_lq, subfolder_gt in zip(subfolders_lq,
             ...
             #   print(img_paths_lq)
                ...
                #print(img_paths_lq,img_paths_gt)
...
class VideoTestDataset2(data.Dataset):
    """Video test dataset.
    ...
    def __init__(self, opt):
        super(VideoTestDataset2, self).__init__()
        self.opt = opt
        self.cache_data = opt['cache_data']
        self.gt_root, self.lq_root = opt['dataroot_gt'], opt['dataroot_lq']
        self.data_info = {
        ...
        if 'meta_info_file' in opt:
            with open(opt['meta_info_file'], 'r') as fin:
                for line in fin:
                    subfolders=line.split(' ')[0]
                    subsub=line.split(' ')[1]
                    
                    subfolders_lq.append(osp.join(self.lq_root, subfolders,subsub))
                    subfolders_gt.append(osp.join(self.gt_root, subfolders,subsub))
            

               # print(subfolders,subsub)
                #subfolders_lq = [
                #    osp.join(self.lq_root, key) for key in subfolders
                #]
                #print(subfolders_lq)
...
class VideoTestDatasetAB(data.Dataset):
    """Video test dataset.
    ...
    def __init__(self, opt):
        super(VideoTestDatasetAB, self).__init__()
        self.opt = opt
        self.cache_data = opt['cache_data']
        self.gt_root, self.lq_root = opt['dataroot_gt'], opt['dataroot_lq']
        self.data_info = {
    ...
    def __getitem__(self, index):
        ...
        img_gt = img_gt[:,0:h//16*16,0:w//16*16]
        imgs_lq = imgs_lq[:,:,0:h//16*16,0:w//16*16]
        c, h, w = img_gt.shape
        
        imgs_lq1 = imgs_lq[:,:,0:h//2,0:w//2]
        imgs_lq2 = imgs_lq[:,:,h//2:h,w//2:w]
        imgs_lq3 = imgs_lq[:,:,0:h//2,w//2:w]
        imgs_lq4 = imgs_lq[:,:,h//2:h,0:w//2]

        return {
            'lq': imgs_lq,  # (t, c, h, w)
            'lq1': imgs_lq1,  # (t, c, h, w)
            'gt': img_gt,  # (c, h, w)
            'lq2': imgs_lq2,  # (t, c, h, w)
            'lq3': imgs_lq3,  # (t, c, h, w)
            'lq4': imgs_lq4,  # (t, c, h, w)
            'folder': folder,  # folder name
            'idx': self.data_info['idx'][index],  # e.g., 0/99
            'border': border,  # 1 for border, 0 for non-border
            'lq_path': lq_path  # center frame
...
class VideoTestDatasetN(data.Dataset):
    """Video test dataset.
    ...
    def __init__(self, opt):
        super(VideoTestDatasetN, self).__init__()
        self.opt = opt
        self.cache_data = opt['cache_data']
        self.gt_root, self.lq_root = opt['dataroot_gt'], opt['dataroot_lq']
        self.data_info = {
